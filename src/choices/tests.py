"""Backport of Django 3.0 enumerations for model field choices tests."""
# pylint: skip-file
import datetime
import decimal
import ipaddress
...
class Suit(choices.IntegerChoices):
    DIAMOND = 1, _("Diamond")
    SPADE = 2, _("Spade")
    HEART = 3, _("Heart")
    CLUB = 4, _("Club")
...
class Vehicle(choices.IntegerChoices):
    CAR = 1, "Carriage"
    TRUCK = 2
    JET_SKI = 3
...
class Gender(choices.TextChoices):
    MALE = "M"
    FEMALE = "F"
    NOT_SPECIFIED = "X"
...
class BackportTests(SimpleTestCase):
    """Test backport is necessary."""

    def test_version_check(self):
        """Django's version is < 3.0."""
        msg = (
            "This backport (choices app) is no longer necessary. The feature "
            "was added in Django 3.0 and the current version of Django is"
            f" '{get_version()}'"
        )
        # check the version of Django is < 3.0
        self.assertLess(VERSION, (3, 0), msg=msg)
...
class ChoicesTests(SimpleTestCase):
    def test_integerchoices(self):
        ...
        self.assertIsInstance(Suit, type(choices.Choices))
    ...
    def test_integerchoices_functional_api(self):
        Place = choices.IntegerChoices("Place", "FIRST SECOND THIRD")
    ...
    def test_textchoices(self):
        ...
        self.assertIsInstance(YearInSchool, type(choices.Choices))
    ...
    def test_textchoices_functional_api(self):
        Medal = choices.TextChoices("Medal", "GOLD SILVER BRONZE")
    ...
    def test_textchoices_blank_value(self):
        class BlankStr(choices.TextChoices):
            EMPTY = "", "(Empty)"
            ONE = "ONE", "One"
    ...
    def test_invalid_definition(self):
        ...
        with self.assertRaisesMessage(TypeError, msg):
            ...
            class InvalidArgumentEnum(choices.IntegerChoices):
        ...
        with self.assertRaisesMessage(ValueError, msg):
            ...
            class Fruit(choices.IntegerChoices):
                APPLE = 1, "Apple"
...
class Separator(bytes, choices.Choices):
...
class Constants(float, choices.Choices):
    PI = 3.141592653589793, "π"
    TAU = 6.283185307179586, "τ"


class Set(frozenset, choices.Choices):
    A = {1, 2}
    B = {2, 3}
    UNION = A | B
    DIFFERENCE = A - B
...
class MoonLandings(datetime.date, choices.Choices):
...
class DateAndTime(datetime.datetime, choices.Choices):
    A = 2010, 10, 10, 10, 10, 10
    B = 2011, 11, 11, 11, 11, 11
    C = 2012, 12, 12, 12, 12, 12


class MealTimes(datetime.time, choices.Choices):
    BREAKFAST = 7, 0
    LUNCH = 13, 0
    DINNER = 18, 30


class Frequency(datetime.timedelta, choices.Choices):
...
class Number(decimal.Decimal, choices.Choices):
    E = 2.718281828459045, "e"
    PI = "3.141592653589793", "π"
    TAU = decimal.Decimal("6.283185307179586"), "τ"


class IPv4Address(ipaddress.IPv4Address, choices.Choices):
    LOCALHOST = "127.0.0.1", "Localhost"
    GATEWAY = "192.168.0.1", "Gateway"
    BROADCAST = "192.168.0.255", "Broadcast"


class IPv6Address(ipaddress.IPv6Address, choices.Choices):
    LOCALHOST = "::1", "Localhost"
    UNSPECIFIED = "::", "Unspecified"


class IPv4Network(ipaddress.IPv4Network, choices.Choices):
    LOOPBACK = "127.0.0.0/8", "Loopback"
    LINK_LOCAL = "169.254.0.0/16", "Link-Local"
    PRIVATE_USE_A = "10.0.0.0/8", "Private-Use (Class A)"


class IPv6Network(ipaddress.IPv6Network, choices.Choices):
...
class CustomChoicesTests(SimpleTestCase):
    ...
    def test_bool_unsupported(self):
        ...
        with self.assertRaisesMessage(TypeError, msg):
            ...
            class Boolean(bool, choices.Choices):
                pass
    ...
    def test_timezone_unsupported(self):
        ...
        with self.assertRaisesMessage(TypeError, msg):
            ...
            class Timezone(datetime.timezone, choices.Choices):
                pass
    ...
    def test_uuid_unsupported(self):
        ...
        with self.assertRaisesMessage(TypeError, msg):
            ...
            class Identifier(uuid.UUID, choices.Choices):
